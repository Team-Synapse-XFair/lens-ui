...
export default function ReportForm() {
    ...
    const handleFileChange = async (e) => {
        ...
        for (const file of files) {
            ...
            try {
                const response = await axios.post('/api/reports/upload-image', formData, {
                    headers: {
    ...
    const onSubmit = async (data) => {
        ...
        if (!respData.success) {
            setServerError(respData.message || 'An error occurred while submitting the report.');
        }
    ...
    return (
        <div className="flex items-center justify-center bg-background">
            <Card className="w-full p-6 pt-1 pb-2 bg-background border-none shadow-none">
                <CardHeader>
                        ...
                        <CardContent className="grid gap-5">
                            {serverError && (
                                <div className="text-sm text-destructive bg-sidebar-primary/10 p-2 rounded">
                                    <AlertCircle className="inline mr-1.5 mb-[0.75]" size={16} />
                                    {serverError}
                                </div>
                            )}
                            <div className='grid gap-2'>
                                <Label htmlFor="title">Title</Label>
                                <Input id="title" {...register('title')} placeholder="Report Title" />
                                {errors.title && <p className="text-sidebar-primary text-sm">{errors.title.message}</p>}
                            </div>
                            <div className='grid gap-2'>
                                <Label htmlFor="description">Description</Label>
                                <Textarea id="description" {...register('description')} placeholder="Detailed description of the issue" rows={4} />
                                {errors.description && <p className="text-sidebar-primary text-sm">{errors.description.message}</p>}
                            </div>
                            <div className="grid gap-2">
                ...
                <CardFooter>
                    <Button onClick={handleSubmit(onSubmit)} disabled={loading} className="w-full">
