...
export default function RegisterForm() {
    ...
    const onSubmit = async (data) => {
        ...
        if (!result) {
            console.log('No response from server during login after registration.');
            router.push('/auth/login');
            return;
        } else if (result.error) {
            setServerError(result.error || 'Login after registration failed. Please try to login manually.');
            return;
        }
                ...
                <Card className="w-full p-6 pt-1 pb-2 bg-card-border/50 border-none">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md space-y-6">
                        <CardContent className="grid gap-7">
                            {serverError && (
                                <div className="text-sm text-destructive bg-sidebar-primary/10 p-2 rounded">{serverError}</div>
                            )}
                            <div className="grid gap-3">
                                <div className='grid gap-2'>
                                    <Label htmlFor="username">Username</Label>
                                    <Input id="username" type="text" placeholder="Enter your username" {...register('username')} />
                                    {errors.username && <p className="text-sidebar-primary text-sm">{errors.username.message}</p>}
                                </div>
                                ...
                                <div className="flex space-x-4">
                                    <div className="flex-1 grid gap-2">
                                        ...
                                        {errors.firstname && <p className="text-sidebar-primary text-sm">{errors.firstname.message}</p>}
                                    </div>
                                    <div className="flex-1 grid gap-2">
                                        <Label htmlFor="lastname">Last Name</Label>
                                        <Input id="lastname" type="text" placeholder="Last Name" {...register('lastname')} />
                                        {errors.lastname && <p className="text-sidebar-primary text-sm">{errors.lastname.message}</p>}
                                    </div>
                                </div>
                                <div className='grid gap-2'>
                                    <Label htmlFor="email">Email</Label>
                                    <Input id="email" type="email" placeholder="Enter your email" {...register('email')} />
                                    {errors.email && <p className="text-sidebar-primary text-sm">{errors.email.message}</p>}
                                </div>
                            </div>
                            <div className="grid gap-3">
                                <div className='grid gap-2'>
                                    ...
                                    {errors.password && <p className="text-sidebar-primary text-sm">{errors.password.message}</p>}
                                </div>
                                <div className='grid gap-2'>
                                    ...
                                    {errors.confirmPassword && <p className="text-sidebar-primary text-sm">{errors.confirmPassword.message}</p>}
                                </div>
                            </div>
                        </CardContent>
                    </form>
                    <CardFooter className="flex flex-col gap-4 mt-2">
                        ...
                        <div className="text-sm text-center text-muted-foreground">
                            Already have an account? <a href="/auth/login" className="text-primary hover:underline">Login here</a>
                        </div>
                    </CardFooter>
                </Card>
        </div>
